...
- `npm run test:review-status` — hand-in en `/api/submit` starten allebei als `pending`, approve/reject, zichtbaarheid op de drie publieke routes, migratie van bestaande rijen naar `approved`, dat de 7-dagen-wandtermijn vanaf `approved_at` loopt (niet vanaf `created_at`), de inhoud van de bevestigingsmails, en dat approve/reject de maker allebei mailen met de juiste inhoud (werkende `/work/:slug`-link resp. de vaste afwijzingstekst zonder reden) (via een lokale Resend-fixture, zelfde patroon als `test:archive-email`)
