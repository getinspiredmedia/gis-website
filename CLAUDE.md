...
- `npm run test:review-status` — hand-in en `/api/submit` starten allebei als `pending`, approve/reject, zichtbaarheid op de drie publieke routes, migratie van bestaande rijen naar `approved`, dat de 7-dagen-wandtermijn vanaf `approved_at` loopt (niet vanaf `created_at`), de inhoud van de bevestigingsmails, en dat approve de maker mailt met een werkende `/work/:slug`-link (via een lokale Resend-fixture, zelfde patroon als `test:archive-email`)
