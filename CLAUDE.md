...
- `POST /api/submit` — publiek submit-formulier; **let op:** het endpoint zelf controleert géén `SUBMIT_TOKEN` (dat gebeurt alleen op de paginaroute `GET /submit/:token` hieronder) — maakt het werk aan met `review_status='pending'`, stuurt "pending approval"-mail naar admin + bevestigingsmail naar artiest (zelfde patroon als `/hand-in/:token`)
