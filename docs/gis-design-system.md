...
- **`.menu a` relies on a page-level reset it doesn't own** — `build/partials/nav.html`'s `.menu a` rule (and its hand-maintained copy in `public/on-view/index.html`) never set `text-decoration` itself; it depends on each host page's own global `a{text-decoration:none}` reset. All 7 build-system pages have that reset, so the partial has always rendered correctly there — but `public/on-view/index.html` never had a global anchor reset (only a `box-sizing` reset), so every `.menu a` link, including the Log in pill, rendered with the browser-default underline until fixed directly in that file. The fix was scoped to `public/on-view/index.html` only; `build/partials/nav.html` was left as-is since it isn't actually broken on any page that currently includes it. Backlog: either give the partial its own `text-decoration:none` so it doesn't depend on the host page, or note it as a hard prerequisite for any future page that includes it.
